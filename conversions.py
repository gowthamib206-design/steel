...
COEFFS_TEMP_TO_UV_LOW = [
    ...
    5.904042111700e-3,       # c2
    -1.325793163600e-6,     # c3
    1.566829190100e-9,      # c4
    -1.694452924000e-12,    # c5
    6.229034709400e-16,     # c6
]
...
class RTDTemperatureTable:
    ...
    rtd_values = [
        18.4932, 18.9258, 19.3580, 19.7899, 20.2215, 20.6526, 21.0834, 21.5139, 21.9439, 22.3737, 22.8031,
...
class ThermocoupleTable:
    """Thermocouple lookup table and conversion methods"""
    
    # Placeholder for thermocouple_values if needed in the future
    thermocouple_values = []
       
    @classmethod
    def get_temperature_from_voltage(cls, voltage: float) -> float:
        """Convert thermocouple voltage (mV) to temperature"""
        ...
        # In the original code, this was using index based on mV. 
        # But we also have polynomial functions. Using the polynomial logic elsewhere.
        # Keeping this for consistency if the table is ever populated.
        if not cls.thermocouple_values:
            # logger.warning("Thermocouple values table is empty, returning approximate value")
            return voltage_uV_to_temperature_C(voltage * 1000.0)
...
def voltage_uV_to_temperature_C(uV):
    """
    Convert voltage (µV) to temperature (°C) using polynomial.
    """
    ...
    v = uV 
    terms = [coeffs[i] * (v ** i) for i in range(len(coeffs))]
    temp_C = sum(terms)
...
def temperature_C_to_voltage_uV(temperature_C: float) -> float:
    ...
    terms = [coeffs[i] * (temperature_C ** i) for i in range(len(coeffs))]
