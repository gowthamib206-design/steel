...
# -------------------------------
    ...
    def _open_port_bg(self, port_name):
        ...
        try:
            ...
            if success:
                ...
                # Open PCM file for recording raw serial data
                try:
                    self._pcm_file = open("test.pcm", "wb")
                    logger.info("Opened test.pcm for recording serial data")
                except Exception as exc:
                    logger.error(f"Failed to open test.pcm: {exc}")
                    self._pcm_file = None
    ...
    def _close_port(self):
        ...
        if pcm_file:
            try:
                pcm_file.close()
                logger.info("Closed test.pcm")
            except Exception as exc:
                logger.error(f"Error closing test.pcm: {exc}")
            self._pcm_file = None
    ...
    def _read_data(self):
        ...
        try:
            ...
            if data:
                # Write raw byte to PCM file
                pcm_file = getattr(self, "_pcm_file", None)
                if pcm_file:
                    try:
                        pcm_file.write(data)
                    except Exception as exc:
                        logger.error(f"Error writing to test.pcm: {exc}")
