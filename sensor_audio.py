...
class DashboardFrame(tk.Frame):
    ...
    def __init__(self,parent, controller):
        ...
        self.raw_log_file = None  # File handle for raw serial data logging
...
# -------------------------------
    ...
    def _open_port_bg(self, port_name):
        ...
        try:
            ...
            if success:
                ...
                # Open PCM file to store raw serial data as-is
                try:
                    self.raw_log_file = open("raw_audio.pcm", "wb")
                    logger.info("PCM file opened: raw_audio.pcm")
                except Exception as exc:
                    logger.error(f"Failed to open PCM file: {exc}")
    ...
    def _close_port(self):
        ...
        if self.raw_log_file:
            try:
                ...
                logger.info("PCM file raw_audio.pcm closed")
            except Exception as exc:
                logger.warning(f"Error closing PCM file: {exc}")
            self.raw_log_file = None
    ...
    def _read_data(self):
        ...
        try:
            ...
            if data:
                # Write raw byte to PCM file as-is
                if self.raw_log_file:
                    try:
                        self.raw_log_file.write(data)
                        self.raw_log_file.flush()
                    except Exception as exc:
                        logger.warning(f"Failed to write to PCM file: {exc}")
